...
async function exportarBackup() {
  if (!db || !currentUser) { showToast('Faça login primeiro','error'); return; }
  showToast('Gerando backup...','success');
  try {
    const uid = currentUser.uid;
    // Busca todas as coleções do usuário
    const [ps, ms, fs] = await Promise.all([
      db.collection('postos').where('uid','==',uid).get(),
      db.collection('manutencoes').where('uid','==',uid).get(),
      db.collection('financeiros').where('uid','==',uid).get()
    ]);

    const backup = {
      versao: '1.0',
      exportadoEm: new Date().toISOString(),
      usuario: { uid: currentUser.uid, email: currentUser.email },
      dados: {
        postos:      ps.docs.map(d => ({ _id: d.id, ...d.data() })),
        manutencoes: ms.docs.map(d => ({ _id: d.id, ...d.data() })),
        financeiros: fs.docs.map(d => ({ _id: d.id, ...d.data() }))
      },
      totais: {
        postos:      ps.size,
        manutencoes: ms.size,
    ...
    a.href     = url;
    a.download = `fueltech-backup-${data}.json`;
    a.click();
    URL.revokeObjectURL(url);

    showToast(`Backup exportado! ${ps.size} postos, ${ms.size} serviços, ${fs.size} lançamentos`,'success');
    closeProfile();
  } catch(e) {
...
function importarBackup(event) {
  ...
  if (!file) return;
  event.target.value = ''; // reset para poder reimportar o mesmo arquivo
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!data.dados || !data.dados.postos || !data.dados.manutencoes || !data.dados.financeiros) {
        showToast('Arquivo inválido — não é um backup do FuelTech','error');
        return;
      }

      _importData = data;
      const d = data.dados;
      const exportDate = data.exportadoEm ? new Date(data.exportadoEm).toLocaleDateString('pt-BR') : '—';

      document.getElementById('import-filename').textContent = file.name;
      document.getElementById('import-preview-text').innerHTML =
        `Este backup contém:<br>` +
        `• <strong>${d.postos.length} postos</strong><br>` +
        `• <strong>${d.manutencoes.length} serviços/manutenções</strong><br>` +
        `• <strong>${d.financeiros.length} lançamentos financeiros</strong><br><br>` +
        `Exportado em: <strong>${exportDate}</strong><br><br>` +
        `Os registros serão <strong>adicionados</strong> ao seu banco atual (nada será apagado).`;
...
async function confirmarImport() {
  if (!_importData || !db || !currentUser) return;
  ...
  try {
    const uid = currentUser.uid;
    const d   = _importData.dados;
    let importados = { postos: 0, manutencoes: 0, financeiros: 0 };

    // Importa em lotes (Firestore tem limite de 500 por batch)
    const batchSize = 400;

    async function importCollection(colName, items) {
      for (let i = 0; i < items.length; i += batchSize) {
        const batch = db.batch();
        const chunk = items.slice(i, i + batchSize);
        chunk.forEach(item => {
          const { _id, ...data } = item;
          // Sempre garante que pertence ao usuário atual
          data.uid = uid;
          // Mantém ID original se possível, para evitar duplicatas em reimportações
          const ref = _id
            ? db.collection(colName).doc(_id)
            : db.collection(colName).doc();
          batch.set(ref, data, { merge: true }); // merge: true não sobrescreve campos extras
        });
        await batch.commit();
        importados[colName] += chunk.length;
      }
    }

    await importCollection('postos',      d.postos);
    await importCollection('manutencoes', d.manutencoes);
    await importCollection('financeiros', d.financeiros);
    ...
    showToast(
      `Importado! ${importados.postos} postos, ${importados.manutencoes} serviços, ${importados.financeiros} lançamentos`,
      'success'
    );
  } catch(e) {
